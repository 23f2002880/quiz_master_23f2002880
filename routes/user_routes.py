...
def submit_quiz(quiz_id):
    ...
    # ✅ Calculate score safely (handle unanswered questions)
    total_score = sum(
        1 for q in correct_answers if str(user_answers.get(str(q.id), 0)) == str(q.correct_option)
    )

    # ✅ Save the score in the database
    new_score = Score(
        quiz_id=quiz_id,
        user_id=user.id,  # ✅ Store the actual user ID (not username)
